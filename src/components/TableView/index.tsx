...
import { NewRowTextInput, NewRowDateInput, NewRowEnumInput } from './NewRowInputs';
              ...
              {newRowData && (
                <tr className="bg-amber-500/10 hover:bg-amber-500/15 transition-colors border-b border-amber-500/30">
                  ...
                  {gridCols.map((colName) => {
                    ...
                    const isEnumCol = isEnumColumn(columnInfo);
                    return (
                      <td 
                        key={colName}
                      ...
                      >
                        {isAutoGeneratedPk ? (
                          ...
                          <NewRowEnumInput
                            value={newRowData[colName] || ''}
                            options={columnInfo?.enum_values ?? []}
                            placeholder={columnInfo?.dflt_value ? 'DEFAULT' : 'Select value'}
                            allowNull={!columnInfo?.notnull}
                            allowDefault={Boolean(columnInfo?.dflt_value)}
                            onChange={(val) => setNewRowData({ ...newRowData, [colName]: val })}
                            onKeyDown={(e) => {
